...
class GetGradientNoPadding(nn.Module):
    ...
    def forward(self, x):
        ...
        def _grad(z):
            zv = F.conv2d(z, self.weight_v, padding=1)
            zh = F.conv2d(z, self.weight_h, padding=1)
            return torch.sqrt(zv * zv + zh * zh + 1e-6)

        return torch.cat([_grad(x0), _grad(x1), _grad(x2)], dim=1)
...
class ISNet_No_Sigmoid(nn.Module):
    def __init__(self, layer_blocks=[4] * 3,
                 channels=[8, 16, 32, 64]):  # layer_blocks = [4] * 3, channels = [8, 16, 32, 64]
        ...
        self.myb2 = TFD(64, 64)
        self.myb3 = TFD(64, 64)
        self.grad_extractor = GetGradientNoPadding()  # 将边缘求取直接嵌入到网络，新增内部梯度模块
...
if __name__ == '__main__':
    # net = ISNet_No_Sigmoid(layer_blocks = [4] * 3,
    #     channels = [8, 16, 32, 64])
